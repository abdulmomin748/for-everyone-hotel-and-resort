...
const Register = () => {
  const {createUser, signinWithPopUp, changeProfile, emailVarification} = useContext(AuthContext);
  ...
  const handleSubmit = event => {
    ...
    .then(result => {
      ...
      handleChangeProfile(name, photoUrl);
  ...
  const handleChangeProfile = (name, photoUrl) => {
    const profile = {
      displayName: name,
      photoURL: photoUrl
    }
    ...
    .then(() => {
      Swal.fire({
        icon: 'success',
        title: 'Succesfully Update Profile',
      })
    })
    .catch(error => {
      Swal.fire({
